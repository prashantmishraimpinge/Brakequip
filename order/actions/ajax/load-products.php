...
<?php foreach (OrderProductsTable::getProductsByCategory($category) as $product): ?>
    <div class="row mod" onmouseover="this.style.background = '#f6f6f6'" onmouseout="this.style.background = '#fff'">
        ...
        <div class="col4"><input id="qty-<?php echo $product->id ?>" type="text" name="qty" class="qty" value="<?php echo ($cart->getQty($product->id) ? $cart->getQty($product->id) : '') ?>" data-value="<?php echo $product->part_number ?>" autocomplete="off" /></div>
        <div class="cb"></div>
    </div>
<?php endforeach; ?>
